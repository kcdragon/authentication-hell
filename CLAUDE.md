...
- **`<base href="/game/">`** in `app/views/layouts/game.html.erb` makes the loader's bare relative asset paths resolve against the static bundle in `public/game/`. That layout intentionally omits the app-wide `javascript_importmap_tags` so the loader's bare paths don't get rewritten. The trade-off: any Hotwire/JS the `/play` view needs must be loaded with a **page-scoped `<script type="importmap">` whose specifiers are absolute, digested `asset_path(...)` URLs** — absolute so the base href leaves them alone. `show.html.erb` does exactly this to load Turbo for the collision toasts (below).
- **Collision toasts (Turbo Streams):** the game POSTs to `/play/collision` (`GamesController#collision`, CSRF skipped for that action since the WASM client can't carry a token — it's still same-origin + session-gated and only broadcasts). The action renders `games/_toast.html.erb` and `Turbo::StreamsChannel.broadcast_append_to`s it onto the page, scoped to `Current.user` so only the colliding player sees it. `show.html.erb` subscribes with `turbo_stream_from Current.user, :toasts` and appends into a fixed bottom-right `#toasts` flex column; the fade-in/auto-dismiss is pure CSS (`toast-in` keyframes in `app/assets/tailwind/application.css`). No Stimulus/ActionCable JS of our own — Turbo does the DOM work.
