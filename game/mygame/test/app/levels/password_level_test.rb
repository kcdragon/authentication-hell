...
class PasswordLevelTest < Minitest::Test
  ...
  def test_a_full_but_unbalanced_password_is_rejected_and_reset
    ...
    assert @level.validation_error_active?(@frame), "the invalid-password banner is showing"
  end
  ...
  def test_a_valid_password_does_not_trigger_a_validation_error
    ...
    refute @level.validation_error_active?(@frame)
  end
  ...
  def test_validation_error_banner_expires_after_its_window
    ...
    stage(all_one_class)
    @level.update(@frame)
    assert @level.validation_error_active?(@frame)

    later = build_frame(player: @player, level: @level,
  ...
  def test_draw_hud_paints_a_slot_for_every_required_character
    ...
    solids_per_slot = 2
    slots = @frame.outputs.solids.count / solids_per_slot
  ...
  def test_draw_shows_the_invalid_password_banner_while_the_error_is_active
    ...
    assert(@frame.outputs.labels.any? { |l| l[:text] == "INVALID PASSWORD" })
  end
