...
# (descending onto the head vs. feet on the ground) picks the outcome.
class EnemyTest < Minitest::Test
  ...
  def test_stomp_defeats_the_enemy_and_bounces_the_player
    ...
    enemy.on_collision(build_args(player: @player, level: level))
  ...
  def test_buffering_enemy_slows_the_player_instead_of_docking_a_heart
    ...
    enemy.on_collision(build_args(player: @player, tick_count: 0))
  ...
  def test_invincible_player_passes_through_unharmed
    ...
    enemy.on_collision(build_args(player: @player, tick_count: 1))
  ...
  def test_melee_off_turns_a_would_be_stomp_into_the_re_auth
    ...
    enemy.on_collision(build_args(player: @player, level: level, tick_count: 0))
