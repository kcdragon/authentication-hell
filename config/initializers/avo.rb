...
Avo.configure do |config|
  ...
  ## == Authentication ==
  # Avo's engine controllers don't include our Authentication concern, so resume
  # the session from the cookie here (the two lines mirror Authentication#resume_session)
  # and admit only super admins. main_app. is needed because this runs in the engine.
  config.current_user_method = proc { Current.user }
  config.authenticate_with do
    ...
    unless Current.user&.super_admin?
      redirect_to main_app.new_session_path, alert: "You don't have access to that page."
    end
  end
